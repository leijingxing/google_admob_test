...
/// 预警处置页面。
class WarningDisposalView extends GetView<WarningDisposalController> {
  const WarningDisposalView({super.key});

  @override
  Widget build(BuildContext context) {
    return GetBuilder<WarningDisposalController>(
      builder: (logic) {
        return Scaffold(
          appBar: AppBar(title: Text(logic.pageTitle)),
          body: _WarningDisposalBody(controller: logic),
        );
      },
    );
  }
...
class _WarningDisposalBody extends StatefulWidget {
  const _WarningDisposalBody({required this.controller});

  final WarningDisposalController controller;

  @override
  State<_WarningDisposalBody> createState() => _WarningDisposalBodyState();
}

class _WarningDisposalBodyState extends State<_WarningDisposalBody>
  ...
  @override
  void didUpdateWidget(covariant _WarningDisposalBody oldWidget) {
          ...
          child: TabBarView(
            ...
            children: List.generate(widget.controller.tabItems.length, (
              ...
              return CustomEasyRefreshList<RiskWarningDisposalItemModel>(
                key: ValueKey('warning-disposal-$tabIndex'),
...
class _WarningFilterSection extends StatelessWidget {
  const _WarningFilterSection({
                  ...
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      suffixIcon: IconButton(
                        onPressed: () => controller.applyKeyword(
...
class _WarningLevelDropdown extends StatelessWidget {
  const _WarningLevelDropdown({
    required this.value,
  ...
  final int? value;
  final List<WarningLevelOption> options;
...
class _WarningDisposalCard extends StatelessWidget {
  const _WarningDisposalCard({required this.item, required this.controller});
  ...
  Widget build(BuildContext context) {
    return AppInfoStatusCard(
      ...
      body: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _infoRow('预警内容', _display(item.description)),
          SizedBox(height: AppDimens.dp6),
          _infoRow('预警等级', controller.warningLevelText(item.warningLevel)),
          SizedBox(height: AppDimens.dp6),
          _infoRow('预警开始时间', _display(item.warningStartTime)),
          SizedBox(height: AppDimens.dp6),
          _infoRow('预警结束时间', _display(item.warningEndTime)),
        ],
      ),
      trailingAction: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          SizedBox(
            ...
            child: OutlinedButton(
              onPressed: () =>
                  WorkbenchRoutes.toWarningDisposalDetail(item: item),
          ...
          if (item.warningStatus == 0) ...[
            ...
            SizedBox(
              ...
              child: OutlinedButton(
                onPressed: () async {
                  final handled = await WorkbenchRoutes.toWarningDisposalHandle(
                    item: item,
                  );
                  if (handled == true) {
