...
/// 预警处置页。
class WarningDisposalHandleView
    extends GetView<WarningDisposalHandleController> {
  const WarningDisposalHandleView({super.key});

  @override
  Widget build(BuildContext context) {
    return GetBuilder<WarningDisposalHandleController>(
      builder: (logic) {
        return Scaffold(
          appBar: AppBar(title: const Text('预警处置')),
            ...
            child: Column(
              children: [
                AppStandardCard(
                  child: Column(
                    children: [
                      _line('预警名称', logic.display(logic.item.title)),
                      _line('预警内容', logic.display(logic.item.description)),
                      _line(
                        '预警开始时间',
