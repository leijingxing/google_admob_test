...
/// 白名单审批页面。
class WhitelistApprovalView extends GetView<WhitelistApprovalController> {
  const WhitelistApprovalView({super.key});

  @override
  Widget build(BuildContext context) {
    return GetBuilder<WhitelistApprovalController>(
      builder: (logic) {
        return Scaffold(
          appBar: AppBar(title: const Text('白名单审批')),
          body: _WhitelistApprovalBody(controller: logic),
        );
      },
    );
  }
...
class _WhitelistApprovalBody extends StatefulWidget {
  const _WhitelistApprovalBody({required this.controller});

  final WhitelistApprovalController controller;

  @override
  State<_WhitelistApprovalBody> createState() => _WhitelistApprovalBodyState();
}

class _WhitelistApprovalBodyState extends State<_WhitelistApprovalBody>
  ...
  @override
  void didUpdateWidget(covariant _WhitelistApprovalBody oldWidget) {
          ...
          child: TabBarView(
            ...
            children: List.generate(widget.controller.tabItems.length, (
              ...
              return CustomEasyRefreshList(
                key: ValueKey('whitelist-approval-$tabIndex'),
...
class _WhitelistApprovalCard extends StatelessWidget {
  const _WhitelistApprovalCard({required this.item, required this.controller});

  final WhitelistApprovalItemModel item;
  final WhitelistApprovalController controller;
    ...
    return AppInfoStatusCard(
      ...
      body: Column(
        ...
        children: [
          ...
          Text(
            '企业名称：${controller.companyText(item)}',
            style: TextStyle(
    ...
    if (item.parkCheckStatus == 0) {
      ...
      buttons.add(
        SizedBox(
          ...
          child: OutlinedButton(
            onPressed: () async {
              final approved = await WorkbenchRoutes.toWhitelistApprovalApprove(
                item: item,
              );
              if (approved == true) {
                controller.onTypeChanged(controller.selectedType);
              }
            },
            style: OutlinedButton.styleFrom(
