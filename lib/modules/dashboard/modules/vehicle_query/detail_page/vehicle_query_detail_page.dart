...
/// 车辆详情页面。
class VehicleDetailPage extends GetView<VehicleQueryDetailController> {
  const VehicleDetailPage({super.key});

  @override
  Widget build(BuildContext context) {
    return GetBuilder<VehicleQueryDetailController>(
      builder: (logic) {
        return Scaffold(
              ...
              child: Column(
                children: [
                  ...
                  Padding(
                    ...
                    child: _VehicleSummaryCard(row: logic.row),
                  ),
                  ...
                  Padding(
                    ...
                    child: FutureBuilder<ComprehensiveDetailCountModel>(
                      ...
                      builder: (context, snapshot) {
                        ...
                        final count = snapshot.data ?? const ComprehensiveDetailCountModel();
                      ...
                      child: ClipRRect(
                        ...
                        child: TabBarView(
                          children: [
                            ...
                            _ViolationRecordTab(controller: logic.statisticsController, row: logic.row),
                            _BlackRecordTab(controller: logic.statisticsController, row: logic.row),
                          ],
                        ),
                      ),
                    ),
...
class _VehicleSummaryCard extends StatelessWidget {
  const _VehicleSummaryCard({required this.row});
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            _emptyDash(row.carNumb),
            style: TextStyle(color: Colors.white, fontSize: AppDimens.sp20, fontWeight: FontWeight.w700),
          ),
        ],
      ),
    );
...
class _DetailCountTabs extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final items = [
      ...
      _DetailCountItem(label: '违规记录', value: count.violationCount, accentColor: const Color(0xFFBC6C14), backgroundColor: const Color(0xFFFFF5E9)),
      _DetailCountItem(label: '拉黑记录', value: count.blackListCount, accentColor: const Color(0xFFB2433B), backgroundColor: const Color(0xFFFFEEEE)),
    ];
        ...
        children: [
          ...
          Expanded(
            child: CustomEasyRefreshList<VehicleBlackRecordModel>(
              refreshTrigger: _refreshTrigger,
              dataLoader: (pageIndex, pageSize) {
                return widget.controller.loadBlackRecords(pageIndex, pageSize, keyword: _keywordController.text.trim(), carNumb: widget.row.carNumb, realName: '', type: 2);
              },
              itemBuilder: (context, item, index) {
                return _RecordCard(
                  title: '联系电话：${_emptyDash(item.userPhone)}',
...
class _RecordCard extends StatelessWidget {
  const _RecordCard({required this.title, required this.rows, this.actions = const <Widget>[], this.footer});
