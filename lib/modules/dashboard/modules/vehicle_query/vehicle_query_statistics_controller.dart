...
class VehicleQueryStatisticsController extends GetxController {
  ...
  static String carCategoryText(int? carCategory) {
    ...
    return DictFieldQueryTool.carTypeLabel(
      carCategory,
      fallback: '$carCategory',
    );
  }
  ...
  Future<void> loadVehicleCountCards() async {
    ...
    result.when(
      success: _applyVehicleCountCards,
      failure: (error) {
  ...
  void _applyVehicleCountCards(List<VehicleCategoryCountModel> sourceData) {
    final byType = <int, VehicleCategoryCountModel>{
      for (final item in sourceData)
        if (item.carCategory != 0) item.carCategory: item,
    };

    countCards = _defaultCountCards().asMap().entries.map((entry) {
      final index = entry.key;
      final card = entry.value;
      final source = index < sourceData.length
          ? sourceData[index]
          : byType[card.type];
